...
public class HeatProviderItem extends Item {
    public HeatProviderItem(Properties properties) {
        super(properties);
    }

    public static int getVariantIndex(ItemStack stack) {
        Integer index = stack.get(ModDataComponents.HEAT_PROVIDER_INDEX.get());
    ...
    public Component getName(ItemStack stack) {
        ...
        return Component.translatable("item.skyresourcereforge.heat_provider." + variant);
    }

    @Override
    public String getDescriptionId(ItemStack stack) {
        String variant = HeatVariants.getName(getVariantIndex(stack));
        return "item.skyresourcereforge.heat_provider." + variant;
    }
}
